...
#define WITHIMPORTFIELDS
  ...
  subroutine InitializeP2(model, importState, exportState, clock, rc)
    ...
    ! exportable field: precipitation_flux
    field = ESMF_FieldCreate(name="pf", grid=gridOut, &
      typekind=ESMF_TYPEKIND_R8, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
...
#define NUOPC_TRACE__OFF
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      file=__FILE__)) &
      return  ! bail out
    call ESMF_StateGet(importState, "sst", sst, &
      rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    call ESMF_TraceRegionEnter("writesst")
    call ESMF_FieldWrite(sst, "sst.nc", variableName="sst", &
      timeSlice=timeSlice, overwrite=.true., rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call ESMF_TraceRegionExit("writesst")
