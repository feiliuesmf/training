...
#define WITHIMPORTFIELDS
    ...
    call NUOPC_Advertise(importState, &
      StandardName="precipitation_flux", name="pf", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine InitializeP2(model, importState, exportState, clock, rc)
    ...
    field = ESMF_FieldCreate(name="pf", grid=gridIn, &
      typekind=ESMF_TYPEKIND_R8, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    ! exportable field: sea_surface_temperature
    sst = ESMF_FieldCreate(name="sst", grid=gridOut, &
      typekind=ESMF_TYPEKIND_R8, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call NUOPC_Realize(exportState, field=sst, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out

    call ESMF_LogWrite("Done realize in Ocn", ESMF_LOGMSG_INFO) 
...
#define NUOPC_TRACE__OFF
    ...
    rc = ESMF_SUCCESS
    call ESMF_FieldFill(sst, dataFillScheme="const", const1=sst_val, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    call ESMF_TraceRegionEnter("temploop")
    temp = 1.098
    do i=1,1000000
       temp = temp * 1.066
    end do
    print *, temp
    call ESMF_TraceRegionExit("temploop")
